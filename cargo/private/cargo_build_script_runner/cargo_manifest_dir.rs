            ...
            if symlinks_used {
                match remove_symlink(&abs_dest) {
                    ...
                    Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                        ...
                        // Retained entry was missing — fall through to copy from src.
                    }
                    Err(e) => {
                        return Err(format!(
...
mod tests {
    ...
    #[cfg(any(target_family = "windows", target_family = "unix"))]
    #[test]
    fn drain_symlinks_cleans_empty_parent_dir() {
        let base = test_dir("drain_sym_empty_parent");
        let output_dir = base.join("runfiles");
        fs::create_dir_all(&output_dir).unwrap();

        let src_file = base.join("throwaway.txt");
        fs::write(&src_file, "data").unwrap();

        let dest = "subdir/throwaway.txt";
        let abs_dest = output_dir.join(dest);
        fs::create_dir_all(abs_dest.parent().unwrap()).unwrap();
        symlink(&src_file, &abs_dest).unwrap();
        ...
        // Both the symlink and its now-empty parent should be removed.
        assert!(!abs_dest.exists());
